...
<body>
    <nav class="container m-0 mt-4">
        <nav class="m-0">
            <div class="nav nav-tabs" id="nav-tab" role="tablist">
                ...
                <button class="nav-link itemHorizontalBar text-muted" id="nav-horizontalBar-tab" data-bs-toggle="tab"
                    data-bs-target="#nav-horizontalBar" type="button" role="tab" aria-controls="nav-horizontalBar"
                    aria-selected="false">HorizontalBar</button>
            </div>
            <button class='float-right mr-5'><a class='btn btn-outline-success' href="javascript:history.back()">Quay
                    lại</a></button>
        </nav>
    ...
    <nav class=' d-none'>
        <?php 
                require '../../config.php';
                $maBKT=$_GET['maBKT'];            
                $sql="SELECT * FROM ket_qua where maBKT like '$maBKT' ORDER BY diem DESC";
                ...
                for($i=0;$i<$result->num_rows;$i++){
                    ...
                    echo "<div class='userName'>$tenUser</div>";
                    echo "<div class='mark'>".$row['diem']."</div>";
                }
            ?>
    </nav>
    </nav>
...
<script type="text/javascript" src='../js/chartMarkOfTestv1.js'></script>
